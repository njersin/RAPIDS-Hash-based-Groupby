    ...
    const size_t current_byte)
{
  ...
  return (d_key_columns[column_idx * num_key_rows + idx] >> shift) & 0xff;
}
    ...
    const uint32_t seed)
{
    ...
    if (idx < num_key_rows) {
        ...
        for (int i = 0; i < message_size; ++i) {
            message = retrieve_byte<T>(d_key_columns, num_key_columns, num_key_rows, idx, i);
            hash = c_crc_x64_32_tab[(hash ^ message)] ^ (hash >> 16);
        }
        d_crc_x64_32_result[idx] ^= hash;
    }
}
                              ...
                              const uint32_t seed)
{
    ...
    crc_x64_32_hash<T><<<dimGrid, dimBlock>>>(d_key_columns, num_key_columns, num_key_rows,
                                      d_crc_x64_32_result, seed);
    cudaDeviceSynchronize();

    //copy result back to host
    cudaMemcpy(h_crc_x64_32_result, d_crc_x64_32_result, num_key_rows * sizeof(uint32_t),
